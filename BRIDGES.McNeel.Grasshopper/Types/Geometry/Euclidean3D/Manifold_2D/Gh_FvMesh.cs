...
{
    /// <summary>
    /// Class defining a grasshopper type for an <see cref="Fv.FvMesh{TPosition}"/>.
    /// </summary>
    public class Gh_FvMesh : GH_Types.GH_Goo<Fv.FvMesh<Euc3D.Point>>, GH_Kernel.IGH_PreviewData
    {
        #region Properties
        ...
        public RH_Geo.BoundingBox Boundingbox
        {
            get
            {
                ...
                foreach (Fv.FvVertex<Euc3D.Point> vertices in Value.GetVertices())
                {
        ...
        {
            this.Value = (Fv.FvMesh<Euc3D.Point>)gh_FvMesh.Value.Clone();
        }

        /// <summary>
        /// Initialises a new instance of <see cref="Gh_FvMesh"/> class from a <see cref="Fv.FvMesh{TPosition}"/>.
        /// </summary>
        /// <param name="fvMesh"> <see cref="Fv.FvMesh{TPosition}"/> to duplicate.</param>
        public Gh_FvMesh(Fv.FvMesh<Euc3D.Point> fvMesh)
        {
            this.Value = (Fv.FvMesh<Euc3D.Point>)fvMesh.Clone();
        }
        ...
        public override string TypeDescription { get { return String.Format($"Grasshopper type containing a {typeof(Fv.FvMesh<Euc3D.Point>)}."); } }
        ...
        {
            return string.Format($"FvMesh with {Value.VertexCount} vertices, {Value.EdgeCount} edges, {Value.FaceCount} faces,");
        }
        ...
        public override bool CastFrom(object source)
        {
            ...
            // Cast a Fv.FvMesh<Euc3D.Point> to a Gh_FvMesh
            if (typeof(Fv.FvMesh<Euc3D.Point>).IsAssignableFrom(type))
            {
                Value = (Fv.FvMesh<Euc3D.Point>)source;
                return true;
            }
            ...
            if (typeof(RH_Geo.Mesh).IsAssignableFrom(type))
            {
                ...
                rh_Mesh.CastTo(out Fv.FvMesh<Euc3D.Point> mesh);
                Value = mesh;
        ...
        public override bool CastTo<T>(ref T target)
        {
            ...
            // Casts a Gh_FvMesh to a Fv.FvMesh<Euc3D.Point>
            if (typeof(T).IsAssignableFrom(typeof(Fv.FvMesh<Euc3D.Point>)))
            {
