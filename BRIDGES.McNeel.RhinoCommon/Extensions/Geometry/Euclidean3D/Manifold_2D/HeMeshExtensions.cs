...
namespace BRIDGES.McNeel.RhinoCommon.Extensions.Geometry.Euclidean3D
{
    /// <summary>
    /// Class defining casts from <see cref="He.HeMesh{TPosition}"/> to <see cref="Rhino"/> objects.
    /// </summary>
    public static class Euclidean3D_HeMesh_ToRhino
    {
        /// <summary>
        /// Casts a <see cref="He.HeHalfedge{TPosition}"/> to a <see cref="RH_Geo.Mesh"/>.
        /// </summary>
        /// <remarks> 
        ...
        public static void ConvertTo(this He.HeMesh<Euc3D.Point> source, out RH_Geo.Mesh target)
        {
            target = new RH_Geo.Mesh();

            // Add Vertices
            foreach(He.HeVertex<Euc3D.Point> vertex in source.GetVertices())
            {
            ...
            // Add Faces
            foreach(He.HeFace<Euc3D.Point> face in source.GetFaces())
            {
                IReadOnlyList<He.HeVertex<Euc3D.Point>> vertices = face.FaceVertices();
    ...
    {
        /// <summary>
        /// Converts a <see cref="RH_Geo.Mesh"/> to a <see cref="He.HeMesh{TPosition}"/>.
        /// </summary>
        ...
        /// <returns> <see langword="true"/> if the cast succeeded, <see langword="false"/> otherwise. </returns>
        public static void ConvertTo(this RH_Geo.Mesh source, out He.HeMesh<Euc3D.Point> target)
        {
            target = new He.HeMesh<Euc3D.Point>();
            ...
            for (int i_TE = 0; i_TE < source.TopologyEdges.Count; i_TE++)
            {
                ...
                He.HeVertex<Euc3D.Point> start = target.GetVertex(startIndex);
                He.HeVertex<Euc3D.Point> end = target.GetVertex(endIndex);
            ...
            */
            ...
            foreach (RH_Geo.MeshNgon ngon in source.GetNgonAndFacesEnumerable())
            {
                ...
                List<He.HeVertex<Euc3D.Point>> vertices = new List<He.HeVertex<Euc3D.Point>>(indices.Length);
