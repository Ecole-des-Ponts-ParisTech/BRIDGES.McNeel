...
{
    /// <summary>
    /// Class defining casts from <see cref="Fv.FvMesh{TPosition}"/> to <see cref="Rhino"/> objects.
    /// </summary>
    public static class Euclidean3D_FvMesh_ToRhino
    {
        /// <summary>
        /// Casts a <see cref="Fv.FvMesh{TPosition}"/> to a <see cref="RH_Geo.Mesh"/>.
        /// </summary>
        /// <remarks> 
        ...
        public static void CastTo(this Fv.FvMesh<Euc3D.Point> source, out RH_Geo.Mesh target)
        {
            target = new RH_Geo.Mesh();

            // Add Vertices
            foreach (Fv.FvVertex<Euc3D.Point> vertex in source.GetVertices())
            {
            ...
            // Add Faces
            foreach (Fv.FvFace<Euc3D.Point> face in source.GetFaces())
            {
                IReadOnlyList<Fv.FvVertex<Euc3D.Point>> vertices = face.FaceVertices();
    ...
    {
        /// <summary>
        /// Converts a <see cref="RH_Geo.Mesh"/> to a <see cref="Fv.FvMesh{TPosition}"/>.
        /// </summary>
        ...
        /// <returns> <see langword="true"/> if the cast succeeded, <see langword="false"/> otherwise. </returns>
        public static void CastTo(this RH_Geo.Mesh source, out Fv.FvMesh<Euc3D.Point> target)
        {
            target = new Fv.FvMesh<Euc3D.Point>();
            ...
            foreach (RH_Geo.MeshNgon ngon in source.GetNgonAndFacesEnumerable())
            {
                ...
                List<Fv.FvVertex<Euc3D.Point>> vertices = new List<Fv.FvVertex<Euc3D.Point>>(indices.Length);
