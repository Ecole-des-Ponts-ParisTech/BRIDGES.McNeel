...
namespace BRIDGES.McNeel.RhinoCommon.Extensions.Geometry.Euclidean3D
{
    ...
    public static class Euclidean3D_Circle_ToRhino
    {
        ...
        public static void CastTo(this Euc3D.Circle source, out RH_Geo.Circle target)
        {
            ...
            Euc3D.Point start = source.PointAt(0.0);
            Euc3D.Vector xAxis = new Euc3D.Vector(source.Centre, start); xAxis.Unitise();
            xAxis.CastTo(out RH_Geo.Vector3d x);

            Euc3D.Vector yAxis = Euc3D.Vector.CrossProduct(source.Normal, xAxis); yAxis.Unitise();
            yAxis.CastTo(out RH_Geo.Vector3d y);

            RH_Geo.Plane plane = new RH_Geo.Plane(origin, x, y);
